...
class FreezyFaceCheckAuth():
    ...
    def check_uri_dlib(self, video_uri, known_faces):
        match = ''
        # 读取视频
        TOTAL_BLINK = 0
        COUNTER = 0
        ...
        while True:
            ...
            if match != 'yes':
                match = self.checkMatchImage(known_faces,frame)
            if match == 'yes':
    ...
    def check_uri_face_recognition(self, video_uri, known_faces):
        match = ''
        # 读取视频
        TOTAL_BLINK = 0
        COUNTER = 0
    ...
    def check_imgfiles_dlib(self, imgfiles, known_faces):
        match = ''
        # 读取视频
        TOTAL_BLINK = 0
        COUNTER = 0
    ...
    def check_imgfiles_face_recognition(self, imgfiles, known_faces):
        match = ''
        # 读取视频
        TOTAL_BLINK = 0
        COUNTER = 0
    ...
    def checkMatchImage(self, known_faces, auth_image):
        result = 'no'
        auth_face = cv2.cvtColor(auth_image, cv2.COLOR_BGR2RGB)
        auth_face_encoding_array = face_recognition.face_encodings(auth_face)
        if len(auth_face_encoding_array) == 0:
            return result
    ...
    def getSureImgEncoding(self,sure_image):
...
if __name__ == '__main__':
    ffca = FreezyFaceCheckAuth()
    frame = cv2.imread('./test/sure.jpg')
    result = ffca.check_uri_dlib('./test/blink.mp4',ffca.getSureImgEncoding(frame))
    print(result)
